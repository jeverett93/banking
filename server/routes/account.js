...
const getAccountByAccountId = async function (account_id) {
    try {
        const result = await pool.query(
            'select * from account a inner join bank_user b on a.userid = b.userid where a.account_id=$1', [account_id]
        );
...
async function getAccountByEmail(email) {
    try {
        const result = await pool.query(
            'select * from account a inner join bank_user b on a.userid = b.userid where b.email=$1', [email]
        );
...
Router.get('/account', authMiddleware, async (req, res) => {
    try {
        ...
        if (result) {
            ...
            res.status(400).send({
                get_error: 'Account details do not exist.'
            });
        }
    } catch (error) {
        res.status(400).send({
            get_error: 'Error while getting details...Try again later.'
        });
    }
});
